...
class Create_external_staff_request extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['title'] = 'Crear solicitud externa - ' . SITE_NAME;  
        ...
        $data['business_units'] = $this->Employer->get_permission_business_units($user->ID);
        ...
        if (!$this->is_valid_data_request() ||
            ...
            $this->load->view('employer/staff_request/create_external_staff_request_view', $data);
            return;
        }
        ...
        $request_id = $this->Staff_request->create_external_staff_request($data);

        if ($request_id) {
            $this->session->set_flashdata('msg', '<div class="alert alert-success"> <a href="#" class="close" data-dismiss="alert">&times;</a> <strong>Listo!</strong> La solicitud ha sido creada con éxito.</div>');
            
            $this->notify_creation_staff_request_by_email($obj_recruiter, $request_id);
        ...
        redirect('employer/staff_request/create_external_staff_request');
    }
    ...
    private function notify_creation_staff_request_by_email($obj_recruiter, $request_id)
    {
