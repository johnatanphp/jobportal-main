...
class Employers extends CI_Controller
{
    public function index()
    {
        ...
        //Pagination starts
        $total_rows = $this->Employer->record_count('tbl_employers');
        $config = pagination_configuration(base_url("admin/employers"), $total_rows, 25, 3, 5, true);
        ...
        $obj_result = $this->Employer->get_all_employers($config["per_page"], $page);
    ...
    {   
        ...
        $data['msg'] = '';
        $obj_row = $this->Employer->get_employer_by_id($id);
    ...
    public function update($id=''){
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $obj_row = $this->Employer->get_employer_by_id($id);
            $obj_cities = $this->City->get_all_cities();
            $obj_countries = $this->Country->get_all_countries();
            $obj_industries = $this->Industry->get_all_industries();
        ...
        $this->Employer->update_employer($id, $employer_array);
    ...
    {   
        ...
        $app_user = $this->Employer->find($id);
        $company = $this->Company->find($app_user->company_ID);
        $employer = $this->Employer->get_employer_by_id_simple($id);

        if ($app_user->is_admin  == 'yes') {
            $this->Company->update_company($app_user->company_ID, $data);
            //$this->Employer->update_employer($id, $data);
        }

        $this->Employer->update($id, $data);

        $config = $this->Email_drafts->email_configuration();
        
        $data_email = [
            'app_user' => $app_user,
            'app_user_sts' => $new_status,
            'company' => $company
        ];

        //Notificar a usuario estado
        $this->email->initialize($config);
        $this->email->clear(TRUE);
        $this->email->from(ADMIN_EMAIL, SITE_NAME);
        $this->email->to($app_user->email);

        $mail_message = load_email_view('email/app_user_status', $data_email);

        $this->email->subject('Cuenta empresa ' . ($new_status == 'active' ? 'Activada' : 'Bloqueada'));
    ...
    public function delete_employer($id=''){
        ...
        $obj_row = $this->Employer->get_employer_by_id($id);
        $this->Employer->delete_employer($obj_row->ID);
        //$this->Company->delete_company($obj_row->company_ID);
        //$this->Posted_job->delete_post_jobs($employer_id);
    ...
    public function search()
    {
        ...
        //Pagination starts
        $total_rows = $this->Employer->search_record_count('tbl_employers',$search_data);
        $config = pagination_configuration_search(base_url("admin/employers/search/?".$url_params), $total_rows, 10, 3, 5, true);
        ...
        $data["total_rows"] = $total_rows;
        //Pagination ends
        $obj_result = $this->Employer->search_all_employers($config["per_page"], $page, $search_data, $wild_card);
        $data['result'] = $obj_result;
    ...
    public function check_email_address(){
        ...
        $rcd = $this->Employer->is_email_already_exists($id, $email);
        echo $rcd;
        exit;   
    }
    ...
    public function login($id)
    {
        ...
        $this->load->library('App/Session/Session_employer');
        $this->session_employer->logout();
        $this->session_employer->create($user);

        redirect('login');
    }

    public function search_by_company($company_id = 0)
    {
        $data['title'] = SITE_NAME . ': Manage Employers';
        $data['msg'] = '';
        ...
        //Pagination starts
        $total_rows = $this->Employer->count_all_employers_by_company_id($company_id, $filters);

        $config = pagination_configuration_search(base_url('admin/employers/search_by_company/' . $company_id), $total_rows, 25, 4, 5, true);
        ...
        $obj_result = $this->Employer->search_all_employers_by_company_id(
            $company_id,
            $filters, 
            $config["per_page"], 
            $page
        ...
        $data['search_data'] = $filters;

        $this->load->view('admin/companies/company_users_view', $data);  
    }
}
