...
class Exam_request_notify_reschedule_seekers extends App_console  
{
    ...
    public function run()
    {
        ...
        foreach ($result as $row) {
            ...
            $mail_message = load_email_view(
                'email/exam_request_notify_reschedule_seeker', 
                $data_email
            );

            $this->email->subject('Cita Reprogramada para ' . $exam_document->name);
            ...
            $sms_text = "Hola " . $seeker->first_name . ", tu cita de " . $exam_document->name . " se ha reprogramado para el " .
