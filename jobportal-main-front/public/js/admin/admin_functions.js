...
function delete_qualification(id){
	var myurl = baseUrl+'admin/qualifications/delete/'+id;
...
function load_qualification_edit_form(id){
	$.getJSON(baseUrl+'admin/qualifications/get_qualification_by_id/'+id, function(data) {
...
function update_institute_status(id){
	var current_status = $("#sts_"+id+" span").html();
	var myurl = baseUrl+'admin/institutes/status/'+id+'/'+current_status;
	$.get(myurl, function (sts) {
		var class_label = 'success';
		if(sts!='active')
			var class_label = 'danger';
...
function delete_institute(id){
	var myurl = baseUrl+'admin/institutes/delete/'+id;
...
function load_institute_edit_form(id){
	...
	$.getJSON(baseUrl+'admin/institutes/get_institute_by_id/'+id, function(data) {
...
function validate_edit_skill_form(the_form){
  ...
  function load_skill_edit_form(id) {
    $.getJSON(baseUrl + 'admin/skills/get_skill_by_id/' + id, function(data) {
        ...
        $('#occupational_group').val(data.occupational_category_id); 

        if (data.skill_favorite == 1 || data.skill_favorite === '1') {
            $('#skill_favorite').prop('checked', true);
        } else {
            $('#skill_favorite').prop('checked', false);
        }
...
//=========Laboral benefits Module===========

function load_lobor_benefit_add_form() {
	$('#add_page_form').modal('show');
}

function validate_edit_labor_benefit_form(form) {
	if ($.trim(form.edit_benefit_name.value) == '') {
		alert("Por favor ingresa el beneficio laboral.");  
		return false;
	}
 }

function validate_add_labor_benefit_form(form) {
	if ($.trim(form.benefit_name.value) == '') {
		alert("Por favor ingresa el beneficio laboral.");  
		return false;
	}
 }

function load_labor_benefit_edit_form(id) {
	
	$.getJSON(baseUrl + 'admin/laboral_benefits/get_benefit_by_id/' + id, function(data) {
		$( '#edit_benefit_name' ).val(data.benefit_name);
		$( '#edit_benefit_active' ).val(data.active);
...
function delete_labor_benefit(id){
	
	var myurl = baseUrl + 'admin/laboral_benefits/delete/' + id;
	var is_confirm = confirm('¿Estás seguro de eliminar este beneficio laboral?');
...
//========= Config module ======== 
    ...
    function sendTestEmail() {
		...
		$( "#email-test" ).prop('disabled', true);
		$( "#btn-send-email" ).prop('disabled', true);

		$.ajax({
			type: "POST",
			url: baseUrl + "admin/config/send_test_email",
			data: data,
			dataType: 'json',
			success: function(response) {
				...
				if (response.success) {
					...
					alert("Email no pudo ser enviado - " + response.error);
				}
			},
		}).always(function() {
			$( "#email-test" ).prop('disabled', false);
