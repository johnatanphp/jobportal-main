...
class Qualifications extends CI_Controller
{
	public function index()
	{
		...
		//Pagination starts
		$total_rows = $this->Qualification->record_count('tbl_qualifications');
		$config = pagination_configuration(base_url("admin/qualifications"), $total_rows, 25, 3, 5, true);
		...
		$obj_result = $this->Qualification->search($config["per_page"], $page);
	...
	{
		if($id!=''){
			$row = $this->Qualification->get_record_by_id($id);
	...
	public function add()
	{
		...
		$this->Qualification->add($qualification_array);
	...
	{	
		...
		$this->Qualification->update($id, $qualification_array);
	...
	{		
		...
		$obj_row = $this->Qualification->delete($id);
		echo 'done';
		exit;
	}
}
