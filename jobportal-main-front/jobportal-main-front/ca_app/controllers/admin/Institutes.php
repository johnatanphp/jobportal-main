...
class Institutes extends CI_Controller {
	public function index(){
		...
		//Pagination starts
		$total_rows = $this->Institute->record_count('tbl_institute');
		
		$config = pagination_configuration(base_url("admin/institutes"), $total_rows, 25, 3, 5, true);
		...
		//Pagination ends
		$obj_result = $this->Institute->get_all_records($config["per_page"], $page);
	...
	public function get_institute_by_id($id=''){
		if($id!=''){
			$row = $this->Institute->get_institute_by_id($id);
	...
	public function add(){
		...
		$this->Institute->add($institute_array);
	...
	public function update(){
		...
		$this->Institute->update($id, $institute_array);
	...
	public function delete($id=''){
		...
		$obj_row = $this->Institute->delete($id);
		echo 'done';
		exit;
	}
}