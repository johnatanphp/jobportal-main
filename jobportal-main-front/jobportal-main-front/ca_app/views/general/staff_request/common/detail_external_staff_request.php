...
<div>
  <style type="text/css">
    ...
    #content-gantt-entrevistas,
    #content-job-profile-staff-request {
      margin-top: 35px;
    }

    #content-gantt-activities, #content-gantt-entrevistas {
      overflow-y: hidden;
      overflow-x: auto;
    }
    ...
    <div id="content-staff-request-form" class="tab-pane fade in active">
      <div style="text-align: right;padding: 10px 0; display:none;">
        <?php if (($request->sts_process == 'pending' || $request->sts_process == 'unassigned') && $is_session_recruiter): ?>
          <a href="<?php echo site_url('employer/staff_request/edit_external_staff_request/e/' . $request->ID); ?>">
            Editar solicitud
          </a>
        <?php endif; ?>
      </div>
      <?php $this->load->view('general/staff_request/common/staff_request_external_labor_profile'); ?>
    </div>
    <?php if (!empty($request->job_profile_ID)): ?>
