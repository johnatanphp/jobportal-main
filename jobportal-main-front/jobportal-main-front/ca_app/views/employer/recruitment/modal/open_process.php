...
<div id="modal-recruitment-confirm-open-process" class="modal" role="dialog">
...
$(function(){
  ...
  $( '#open-rs-confirm-open-process' ).click(function(){  
      const data = {
          job_id: $( "#job_id" ).val()
      };

      const url = "<?php echo site_url('employer/recruitment_processes/open_process'); ?>";
      
      $( '#modal-recruitment-confirm-open-process .modal-content' ).html('<div class="modal-header">Cargando...</div>');
      $( '#modal-recruitment-confirm-open-process' ).modal('show');

      $.get(url, data, function(response) {
        $( '#modal-recruitment-confirm-open-process .modal-content' ).html(response);
      })
      .fail(function(){
