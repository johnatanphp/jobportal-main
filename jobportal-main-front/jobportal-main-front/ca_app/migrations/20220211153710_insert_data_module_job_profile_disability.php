...
class Migration_Insert_data_module_job_profile_disability extends CI_Migration
{
    public function up()
    {
        $section = [
		...
		foreach ($temas as $tema => $_subtemas) {
			...
			$this->db->insert('tbl_job_profile_disability_themes', [
				'name' => $tema,
				'active' => 1
			]);
			...
			foreach ($_subtemas as $_subtema => $_sections) {
				...
				$this->db->insert('tbl_job_profile_disability_subthemes', [
					'name' => $_subtema,
					'active' => 1,
					'theme_id' => $tema_id
				]);	
				...
				foreach ($_sections as $_section => $_items) {
					...
					$this->db->insert('tbl_job_profile_disability_sections', [
						'name' => $_section,
						'active' => 1,
					...
					foreach ($_items as $_item) {
						...
						$this->db->insert('tbl_job_profile_disability_items', [
							'name' => $_item,
							'active' => 1,
		...
		foreach ($items as $_item => $info) {
			...
			$this->db->update('tbl_job_profile_disability_items', [
    ...
    {
        $this->db->truncate('tbl_job_profile_disability');
        $this->db->truncate('tbl_job_profile_disability_items');
        $this->db->truncate('tbl_job_profile_disability_sections');
        $this->db->truncate('tbl_job_profile_disability_subthemes');
        $this->db->truncate('tbl_job_profile_disability_themes');
    }
}
