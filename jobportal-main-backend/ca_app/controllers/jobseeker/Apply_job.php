...
class Apply_Job extends CI_Controller {
	...
	{
		$data['title'] = SITE_NAME.' : Solicitar el empleo';
		$data['msg']='';
		...
		$row = $this->Posted_job->get_active_posted_job_by_id($this->input->post('jid'));
		...
		$is_already_applied = $this->Applied_jobs->count_applied_job_by_seeker_and_job_id($this->session->userdata('user_id'), $this->input->post('jid'));
		...
		$answers_data = !empty($this->input->post('answers')) ? $this->input->post('answers') : array();


		$this->Applied_jobs->add_applied_job($job_array, $answers_data);
		
		//incrementar
		
		//Sending email
		$row_email = $this->Email->get_records_by_id(5);
		$config = array();
		$config['wordwrap'] = TRUE;
		$config['mailtype'] = 'html';
		
		$data_array = $this->Posted_job->get_active_posted_job_by_id($this->input->post('jid'));
		...
		$mail_message = $this->Email_drafts->apply_job($seeker_id, $row_email->content, $data_array);
