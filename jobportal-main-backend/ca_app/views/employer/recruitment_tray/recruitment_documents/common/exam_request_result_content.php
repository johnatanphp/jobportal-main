...
<div style="padding: 10px 0;">
	<div class="document-section-title">
	  <?php echo $document->name . ' (' . count($exam_request_results). ')'; ?>
	</div> 
	<div>
    ...
    <?php if ($exam_request_results): ?>
      <div style="padding: 8px 0;">
        <?php foreach ($exam_request_results as $index => $file): ?>
            ...
            <div class="attach-file-item-content">
              <div class="attach-file-item-body">
                ...
                <div class="attach-body-content">
                  <?php if ($file->result_file): ?>
                    <div class="file-link">  
                      <a href="<?php echo file_url($file->result_file); ?>" target="_blank">
                        Ver documento
                      </a>
                    </div>
                  <?php else: ?>
                    <div class="file-desc">Sin certificado</div>
                  <?php endif; ?>
                  <div class="file-date"><?php echo date('d M Y', strtotime($file->result_date));?></div>
                  <div class="file-desc"><?php echo $file->result_name ? $file->result_name : ''; ?></div>
                </div>
              </div>
              <div class="attach-file-item-trash">
                <button class="btn btn-default btn-trash-document" 
                        data-file-id="<?php echo $tray_candidate->status_id == 3 || $file->result_origin != 'Adjunto' ? 0 : $file->id; ?>"
                        <?php echo $tray_candidate->status_id == 3 || $file->result_origin != 'Adjunto' ? 'disabled' : ''; ?>>
  ...
  $( '#file-recruitment-documents' ).change(function(e){
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        if (res.status) {
          loadContentExamRequestResults("<?php echo $process_id; ?>", "<?php echo $candidate_id; ?>", "<?php echo $document->key; ?>");
