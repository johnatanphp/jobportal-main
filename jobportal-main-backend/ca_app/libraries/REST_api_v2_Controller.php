 ...
 * @version         3.0.0
 */
abstract class REST_api_v2_Controller extends CI_Controller {
     ...
     */
    public function __construct($config = 'rest', $extra_config = [])
    {
        parent::__construct();
        ...
        $this->get_local_config($config, $extra_config);
        ...
        if ($rest_auth === 'basic') {
            $this->_prepare_authorization_basic();
        }
        else if ($rest_auth === 'bearer_token')
        {
            $this->_prepare_authorization_bearer_token();
        }
     ...
     * @param $config_file
     */
    private function get_local_config($config_file, $extra_config = [])
    {
    ...
    protected function _parse_options()
    {
        ...
        parse_str((string)parse_url($this->input->server('REQUEST_URI'), PHP_URL_QUERY), $options);
    ...
    private function _prepare_authorization_bearer_token()
    {
        if ($this->config->item('load_employer_by_default')) {
            return;
        }

        $bearer_token = trim($this->get_bearer_token());
       
        if ($bearer_token == '') {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE, 
                $this->config->item('rest_message_field_name') => 'Token no es válido'
            ], self::HTTP_UNAUTHORIZED);
        }

        $user_token = $this->rest->db->get_where('tbl_api_user_tokens', ['access_token' => $bearer_token, 'revoked' => 0])->row();
        ...
        if ($user_token->app_user_id) {        
            $user = $this->rest->db->get_where('tbl_api_users', ['id' => $user_token->app_user_id, 'active' => 1])->row();

            if (!$user) {
                $this->response([
                    $this->config->item('rest_status_field_name') => FALSE, 
                    $this->config->item('rest_message_field_name') => 'Usuario app no válido'
                ], self::HTTP_UNAUTHORIZED);
            }
        }
        
        $date_now = new DateTime('now');
        $date_expires = new DateTime($user_token->expires_at);
       
        //Verificar si el token ha expirado
        if ($date_now->getTimestamp() > $date_expires->getTimestamp()) {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE, 
                $this->config->item('rest_message_field_name') => 'Token expirado'
            ], self::HTTP_UNAUTHORIZED);
        }

        if ($user_token->app_user_id) {
            $this->_load_config_authorization($user);
        }   
    }

    private function _prepare_authorization_basic()
    {
        // Returns NULL if the SERVER variables PHP_AUTH_USER and HTTP_AUTHENTICATION don't exist
        $username = $this->input->server('PHP_AUTH_USER');
        $http_auth = $this->input->server('HTTP_AUTHENTICATION') ?: $this->input->server('HTTP_AUTHORIZATION');
        ...
        $user = $this->rest->db->get_where('tbl_api_users', ['username' => $username, 'password' => $password, 'active' => 1])->row();
    ...
    private function _load_config_authorization($user)
    {
        $this->config->set_item('allowed_cors_origins', explode(",", $user->allowed_cors_origins));
        $this->config->set_item('rest_valid_logins', [$user->username => $user->password]);
    }
