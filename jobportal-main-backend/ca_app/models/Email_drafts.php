<?php
class Email_drafts extends CI_Model
{
    public function __construct()
    {
     ...
     public function email_configuration()
     {
        ...
        if (SMTP == true) {
            ...
            if ($this->config->item('smtp_crypto')) {
    ...
    public function send_message_to_candidate($email_body,$message,$row_jobseeker,$row_employer){
        ...
        $email_body=replace_string('{COMPANY_PROFILE_LINK}',base_url('companies/'.$row_employer->company_slug),$email_body);
        return $email_body;
    }
    ...
    public function add_employer($new_employer_name, $email, $password, $site_url_login, $email_body){
        
        $email_body = replace_string('{NEW_EMPLOYER_NAME}', $new_employer_name, $email_body);
        $email_body = replace_string('{EMAIL}', $email, $email_body);
        $email_body = replace_string('{PASSWORD}',$password, $email_body);
        $email_body = replace_string('{SITE_URL_LOGIN}', $site_url_login, $email_body);
