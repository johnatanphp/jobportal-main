...
class Search extends CI_Controller {
	...
	public function search_filter($segment_array){
		...
		if($total_segments=3){
			...
			$total_rows = $this->Posted_job->count_ft_job_search_filter_3($this->uri->segment(2),$this->uri->segment(3),$this->uri->segment(4));
			...
			$result = $this->Posted_job->ft_job_search_filter_3($this->uri->segment(2),$this->uri->segment(3),$this->uri->segment(4), $pagenation['per_page'],$pagenation['page']);
		}
		else{
			if($total_segments==2){
	...
	public function cities_search($city_name){
		...
		$total_rows = $this->Posted_job->count_records_by_city($city_name);
		$config = pagination_configuration(base_url("search/".$city_name), $total_rows, 20, 3, 5, true);
		$pagenation = $this->create_pagination($config, $paging);
		$result = $this->Posted_job->job_search_by_city($city_name, $pagenation['per_page'],$pagenation['page']);
		return array ('total_rows' => $total_rows, 'result' => $result, 'pagenation' => $pagenation);
	}
