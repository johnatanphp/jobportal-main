...
class Job_profile_detail_export
{
    private $spreadsheet;
    private $job_profile;
    private $filters;
    ...
    {        
        ...
        $this->job_profile = $this->Job_profile->find($this->filters['id']);
    }
    ...
    public function build_description()
    {
        ...
        $charge = $this->Job_charge->get_job_charge_by_id($jp->job_charge_ID);
        $study_grade_req = $this->Qualification->get_record_by_id($jp->study_grade_req);
        
        $study_grade_min = $this->Qualification->get_record_by_id($jp->study_grade_min);
        ...
        $row = [
            ...
            $jp->risk_criteria, //Criterio de riesgo
            isset($study_grade_req['text']) ? $study_grade_req['text'] : '',
            $jp->education_req_detail,
            isset($study_grade_min['text']) ? $study_grade_min['text'] : '',
            $jp->education_min_detail,
            $jp->education,
            ($this->Work_experience->find(['code' => $jp->experience]))->name,
            $jp->experience_detail 
        ];
    ...
    {
        $jp = $this->job_profile;
        $jp_id = $this->filters['id'];
        $skills = $this->Job_profile->get_skills_by_job_profile_id($jp_id);
    ...
    {
        $jp_id = $this->filters['id'];
        $responsibilities = $this->Job_profile->get_responsibilities_by_job_profile_id($jp_id);
    ...
    public function build_resources()
    {
        ...
        foreach ($resources_types as $resource_key => $resource_name) {
            ...
            $resource_row = $this->Job_profile->get_resource_by_name($resource_key, $jp_id);
    ...
    public function build_disabilities()
    {
        $jp = $this->job_profile;
        $jp_id = $this->filters['id'];
        $disabilities = $this->Job_profile->get_results_disability($jp_id);
    ...
    public function build_disabilities_valorization()
    {
        $jp = $this->job_profile;
        $jp_id = $this->filters['id'];
        $disabilities = $this->Job_profile->disability_values($jp_id);
    ...
    public function build_factor_valuations()
    {
        ...
        $factor_valuations = $this->Job_profile->get_factor_valuations($jp_id);
        ...
        foreach ($factor_valuations as $row_factor) {
            $row = [
                $row_factor['name'],
                $row_factor['jp_factor_level'] ? $row_factor['jp_factor_level'] : '-', 
                $row_factor['jp_factor_name'] ? $row_factor['jp_factor_name'] : '-', 
                $row_factor['jp_factor_grade'] ? $row_factor['jp_factor_grade'] : '-',
                $row_factor['jp_factor_score'] ? $row_factor['jp_factor_score'] : '-',
            ];
    ...
    public function build_factor_valuations_total()
    {
        ...
        $factor_valuation_total = $this->Job_profile->get_factor_total_score($jp_id);
    ...
    public function build_salary_structure()
    {
        ...
        $company = $this->Company->find($jp->company_id);
        ...
        $benefits = $this->Job_profile->get_benefits($jp->company_id, $jp_id);
        ...
        if ($jp->factor_differentiating == 'Otro') {
            $rows[] = [
                'Factor diferenciador Otro',
                $jp->factor_differentiating_other
            ];
        }

        $occupational_category = $this->Occupational_category->find($jp->occupational_category_id);
