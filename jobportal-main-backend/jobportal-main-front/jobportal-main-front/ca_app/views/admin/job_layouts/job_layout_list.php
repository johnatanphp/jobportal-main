...
<aside class="right-side"> 
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Layouts de puesto</li>
    </ol>
  </section>
  ...
  <section class="content">
    <?php if ($this->session->flashdata('added_action') == true): ?>
        <div class="message-container">
            ...
            <h4>Nuevo layout de puesto creado con éxito</h4>
          </div>
        </div>
    <?php endif; ?>
    ...
    <?php if ($this->session->flashdata('update_action') == true): ?>
      <div class="message-container">
      	<div class="callout callout-success">
          <h4>Layout de puesto actualizado con éxito</h4>
        </div>
      </div>
    <?php endif; ?>
    <div class="row">
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            ...
            <!--Pagination-->
              <div class="pull-right" style="padding: 10px;text-decoration:underline;">
                <a href="<?php echo site_url('admin/job_layouts/create'); ?>">
                  Nuevo
                </a>
                <a href="<?php echo site_url('admin/job_layouts/alert_emails'); ?>" style="padding: 10px;text-decoration:underline;display:none;">
                  Gestión alertas
                </a>
                <a id="job-layout-filter" href="#"
                   data-toggle="modal" 
                   data-target="#modal-job-layouts-filter" style="padding: 10px;text-decoration:underline;">
                  Filtrar
                </a>
          ...
          <div class="box-body table-responsive">
            <table id="tbl-job-layouts" class="table table-bordered table-hover">
              <thead>
                <tr>
                  <th></th>
                  <th>
...
<!-- Modal -->
<div id="modal-job-layouts-filter" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <?php echo form_open('admin/job_layouts/search', array('method' => 'get', 'id' => 'form-filter')); ?>
...
<div id="modal-export" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <!-- Modal content-->
    <?php echo form_open('admin/job_layouts/export_excel_list', ['method' => 'get', 'id' => 'form-job-layout-export']); ?>
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Exportar layouts</h4>
        </div>
        <div class="modal-body">
  ...
  $(function(){
    ...
    function search(params) 
    {
      $( '#tbl-job-layouts' ).DataTable({
        ...
        "bLengthChange": false,
        ajax: {
          url: "<?php echo site_url('admin/job_layouts/search'); ?>",
          type: 'GET',
          data: params
        },
        columns: [
            ...
            {data:'code_integration', 'className': 'style_td text-left'},
          ...
          },          
          {
            data: null, render:function(data) {
              ...
              return `
              <a href="#" class="btn btn-xs ${btnClass} js-update-sts" data-job-layout-id="${data.id}">
                ${btnText}
              </a>
              `;
            }, 'className': 'style_td text-left'
          },
          {
            data: null, render:function(data) {
              ...
              urlEdit = "<?php echo site_url('admin/job_layouts/edit/'); ?>" + data.id;
              urlShow = "<?php echo site_url('admin/job_layouts/show/'); ?>" + data.id;
    ...
    $( "#job-layouts-filter" ).click(function(){
      $( '#modal-job-layouts-filter' ).modal('show');
    });

    function update_sts(btn) {   
      $(btn).attr({'disabled': true});
      var job_layout_id = $(btn).data('job-layout-id');
      var data = {
        id : job_layout_id
      };

      var url = "<?php echo site_url('admin/job_layouts/update_sts'); ?>"
    ...
    $( '#btn-export' ).click(function(){
      var rows = $( '#tbl-job-layouts' ).DataTable().column(0).checkboxes.selected();

      if (rows.length == 0) {
        toastr["error"]('Por favor seleccione al menos 1 registro');
        return;
      }
    ...
    $( '#form-job-layout-export' ).submit(function(e){
      
      var rows = $( '#tbl-job-layouts' ).DataTable().column(0).checkboxes.selected();
      var data = $( '#tbl-job-layouts' ).DataTable().column(0).checkboxes.selected().data();
      ids = [];
      var names = [];
      ...
      for (let i = 0; i < ids.length; i++) {
        for (let j = 0; j < data.length; j++) {
          if (ids[i] == data[j].id) {
            names.push(removeAccents((data[j].code ? data[j].code : '')) + " " + removeAccents(data[j].job_title));
          } 
        }
      }
    ...
    $( '#form-filter' ).submit(function(e){
      ...
      search({
        company_id: $( '#modal-job-layouts-filter select[name=company_id]').val(),
        requested: $( '#modal-job-layouts-filter select[name=requested]').val(),
        status: $( '#modal-job-layouts-filter select[name=status]').val(),
      });

      $( '#modal-job-layouts-filter' ).modal('hide');
      return false;
    });
