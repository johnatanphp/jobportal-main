...
class Process_jobs extends CI_Controller 
{    
    ...
    public function search()
    {
        ...
        $data['title'] = 'Reclutamiento - ' . SITE_NAME;
        ...
        //Pagination starts
        $total_rows = $this->count_all_posted_jobs_by_company_id(
        ...
        //Jobs by company
        $result_posted_jobs = $this->search_posted_jobs_by_company_id(
            $obj_employer->company_ID, 
            $filters, 
            $config["per_page"], 
            $page
        ...
        $this->load->view('employer/recruitment/process_jobs/jobs', $data);
    }

    private function search_posted_jobs_by_company_id(
        $company_id, 
        $filters, 
        $per_page, 
        $page
    ) 
    {
       ...
       if ($expired == 'yes') {
        ...
        if ($query != '') {
            if (is_numeric($query)) {
                $this->db->group_start();
                $this->db->where('recruitment_process.id', $query);
                $this->db->or_where('sr.ID', $query);
                $this->db->group_end();
            }
        ...
        $this->db->group_start();
        $this->db->where('pj.request_ID IS NULL');
        $this->db->or_where_in('sr.request_model_id', [1, 2, 3]);
        $this->db->group_end();
    ...
    public function job_candidates($job_id = 0)
    {
        ...
        $data['ads_row'] = $this->ads;
        $data['title'] = 'Postulaciones - ' . SITE_NAME;

        //Pagination starts
        $total_rows = $this->Posted_job->count_search_candidates_for_job(
            $job_id
        );
        ...
        $candidates = $this->Posted_job->search_candidates_for_job(
            $job_id,
            $config["per_page"], 
            $page
        );
        ...
        $this->load->view('employer/recruitment/process_jobs/job_candidates', $data);
    }

    public function get_job_questions($job_id = 0)
    {
        $result_job_questions = $this->Posted_job->get_form_job_questions($job_id);

        echo json_encode(array(
            'result_job_questions' => $result_job_questions
        ));
    }
}
