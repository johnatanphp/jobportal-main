<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class Recruitment_process_candidate_add_email 
{
    public function __get($var)
    {
        return get_instance()->$var;
    }

    public function send($request_id, $candidate_id)
	{
        $candidate = $this->Job_seeker->find($candidate_id);

        $request = $this->Staff_request->find($request_id);

        // Plantilla de registro candidato en proceso
        $this->db->where('id', 5);
        $template_email = $this->db->get('tbl_email_applicant_phase')->row();

        if (!$template_email) {
            return;
        }
     
		$config = $this->Email_drafts->email_configuration();
		$this->email->initialize($config);
		$this->email->clear(TRUE);
		$this->email->from(ADMIN_EMAIL, SITE_NAME);
		$this->email->to($candidate->email);
		$this->email->subject($template_email->subject);
		$this->email->message($this->build_message_view($request, $candidate, $template_email));     
		//Send email
		$this->email->send();
	}

    public function build_message_view($request, $candidate, $template_email)
    {   
        $job = $this->Posted_job->find([
            'request_ID' => $request->ID
        ]);

        $url_link = $job && $job->job_slug ? site_url('jobs/' . $job->job_slug) : site_url('login');

        $body = str_replace([ 
            '{{candidate_name}}', 
            '{{job_title}}',
            '{{url_link}}',
        ],
        [
            $candidate->first_name,
            $request->job_title,
            $url_link
        ],
            $template_email->body
        );
    
        $data_email = [
            'body' => $body
        ];
        
        return load_email_view('email/notify_stage_candidate', $data_email);
    }
}
