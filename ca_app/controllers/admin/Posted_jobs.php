...
class Posted_Jobs extends CI_Controller {
	public function index(){
		...
		//Pagination starts
		$total_rows = $this->Posted_job->record_count('tbl_post_jobs');
		$config = pagination_configuration(base_url("admin/posted_jobs"), $total_rows, 25, 3, 5, true);
		...
		$obj_result = $this->Posted_job->get_all_posted_jobs($config["per_page"], $page);
	...
	public function details($id=''){
		...
		$data['msg'] = '';
		$obj_row = $this->Posted_job->get_posted_job_by_id($id);
		$data['row'] = $obj_row;
	...
	public function delete_posted_job($id=''){
		...
		$obj_row = $this->Posted_job->get_posted_job_by_id($id);
		$this->Posted_job->delete_posted_job($obj_row->ID);
		$this->Applied_jobs->delete_applied_job_by_posted_job_id($obj_row->ID);
		echo 'done';
		exit;
	}
	...
	public function search($search_title='te_', $search_company='co_', $search_city='ct_', $search_featured='fe_', $search_sts='st_'){
		...
		//Pagination starts
		$total_rows = $this->Posted_job->search_record_count($search_data);
		...
		$data["links"] = $this->pagination->create_links();
		//Pagination ends
		$obj_result = $this->Posted_job->search_all_posted_jobs($config["per_page"], $page, $search_data);
		$data['result'] = $obj_result;
	...
	public function jobs_by_company($employer_id=''){
		...
		if($employer_id==''){
			redirect('admin/posted_jobs');
			exit;
		}
		
		//Pagination starts
		$total_rows = $this->Posted_job->count_records('tbl_post_jobs','employer_ID',$employer_id);
		$config = pagination_configuration(base_url("admin/posted_jobs/jobs_by_company/"), $total_rows, 50, 3, 5, true);
		
		$this->pagination->initialize($config);
        $page = (int)($this->uri->segment(2)) ? $this->uri->segment(3) : 0;
		$page_num = $page-1;
		$page_num = ($page_num<0)?'0':$page_num;
		$page = $page_num*$config["per_page"];
		$data["links"] = $this->pagination->create_links();
		...
		$obj_result = $this->Posted_job->get_posted_job_by_employer_ID($employer_id, $config["per_page"], $page);
	...
	public function latest_job_by_company($employer_id=''){
		...
		$obj_result = $this->Posted_job->get_posted_job_by_employer_ID($employer_id, 1, 0);
		$row= $obj_result[0];
		if($row)
	...
	public function job_by_id($job_id=''){
		...
		$obj_row = $this->Posted_job->get_posted_job_by_id($job_id);
		if($obj_row)
	...
	public function job_activation_email_to_employer($job_id){
		...
		$mail_message = $this->Email_drafts->job_activation_email($row_email->content, $row);
