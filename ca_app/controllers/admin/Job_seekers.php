...
class Job_Seekers extends CI_Controller {
	public function index(){
		...
		//Pagination starts
		$total_rows = $this->Job_seeker->record_count('tbl_job_seekers');
		$config = pagination_configuration(base_url("admin/job_seekers"), $total_rows, 25, 3, 5, true);
		...
		$obj_result = $this->Job_seeker->get_all_job_seekers($config["per_page"], $page);
	...
	public function details($id=''){
		...
		$data['msg'] = '';
		$obj_row = $this->Job_seeker->get_job_seeker_by_id($id);
		$obj_result = $this->Job_seeker->get_all_applied_jobs_by_seekers_ID($id,50,0);
		//print_r($obj_result);
		$data['result'] = $obj_result;
		$data['row'] = $obj_row;
	...
	public function update($id=''){
		...
		$this->Job_seeker->update_job_seeker($id, $job_seeker_array);
	...
	public function delete_job_seeker($id=''){
		...
		$obj_row = $this->Job_seeker->get_job_seeker_by_id($id);
		$this->Job_seeker->delete_job_seeker($obj_row->ID);
		$this->Applied_jobs->delete_applied_job_by_seeker_id($obj_row->ID);
	...
	*/
	...
	public function search($search_name='nm_', $search_email='em_', $search_gender='ge_', $search_city='ct_'){
		...
		//Pagination starts
		$total_rows = $this->Job_seeker->search_record_count('tbl_job_seekers',$search_data);
		...
		$data["links"] = $this->pagination->create_links();
		//Pagination ends
		$obj_result = $this->Job_seeker->search_all_job_seekers($config["per_page"], $page, $search_data);
		$data['result'] = $obj_result;
	...
	public function applied_jobs_list($seeker_id=''){
		...
		//Pagination starts
		$total_rows = $this->Job_seeker->count_records('tbl_seeker_applied_for_job', 'seeker_ID', $seeker_id);
		...
		$obj_result = $this->Job_seeker->get_all_applied_jobs_by_seekers_ID($seeker_id, $config["per_page"], $page);
		$data['result'] = $obj_result;
	...
	{	
		...
		$seeker = $this->Job_seeker->find($id);
