...
class Form_ec extends CI_Controller
{
    ...
    public function index($contract_document_type_id = 0, $process_id = 0)
    {
        ...
        $entry_form_data = $this->Entry_form_ec->get_last_record_by_seeker_id($seeker_id);
        ...
        $data = [
            'title' => "Ficha de datos del trabajador - " . SITE_NAME,
            'ads_row' => $this->ads,
            'jobseeker' => $jobseeker,
            'entry_form' => $entry_form_data,
            'process_country' => $process_country,
            'rightful_claimants' => $this->Entry_form_ec->get_rightful_claimants(@$entry_form_data->entry_form_id),
        ...
        $this->load->view('jobseeker/entry_form/ec/form', $data);
    }
    ...
    public function show($contract_document_type_id = 0, $process_id = 0)
    {
        ...
        if (!$entry_form) {
            redirect('jobseeker/entry_form/ec/form_ec/index/' . $contract_document_type_id . '/' . $process_id);
        }

        $this->load->library('Entry_form/ec/Entry_form_data_show_ec_service');
        $data = $this->entry_form_data_show_ec_service->get_data($process_id, $seeker_id);
        $data['ads_row'] = $this->ads;
        $data['contract_document_type'] = $contract_document_type;
        $this->load->view('jobseeker/entry_form/ec/show', $data);
    }
    ...
    public function save()
    {
        ...
        if ($entry_form) {
            ...
            $this->db->update('tbl_entry_form_ec', $data_entry_form);
        } else {
            // Registrar ficha de ingreso tabla base
            $this->db->insert('tbl_entry_form', $data_entry_form_base);
            $entry_form_id = $this->db->insert_id();

            // Registrar ficha de ingreso tabla datos por pais
            $data_entry_form['entry_form_id'] = $entry_form_id;
            $this->db->insert('tbl_entry_form_ec', $data_entry_form);
        }

        //Guardar derechohabientes
        $this->db->where('entry_form_id', $entry_form_id);
        $this->db->delete('tbl_entry_form_ec_rightful_claimants');
        ...
        foreach ($rightful_claimants as $person) {
            ...
            $this->db->insert('tbl_entry_form_ec_rightful_claimants', $data_person);
        }
        ...
        echo json_encode([
            'status' => true,
            'data' => [
                'redirect_url' => site_url('jobseeker/entry_form/ec/form_ec/show/' . $contract_document_type_id . '/' . $process_id)
            ]
        ]);
    }
}
