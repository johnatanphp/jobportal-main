...
class Documentos extends REST_Controller 
{
    ...
    {
        $list_num_doc_iden = (array)$this->get('postulante_id');

        if (empty($list_num_doc_iden)) {
            $this->response([
                'status' => false,
                'message' => 'Se debe ingresar el campo postulante_id',
                'data' => []
            ], 200);
            return;
        }
    ...
    {   
        ...
        $this->db->where_in('ID', $list_num_doc_iden);
    ...
    private function validation_competence($seeker_ids)
    {
        ...
        $this->db->where_in('seekers.ID', $seeker_ids);
